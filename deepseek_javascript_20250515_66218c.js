...
function loadStudySessions(uid) {
    ...
    dbRef.child('studySessions')
        .orderByChild('userId')
        .equalTo(uid)
        .on('value', handleSessionsUpdate);
}

/**
 * Handle study sessions data updates
 */
function handleSessionsUpdate(snapshot) {
    const sessions = [];
...
async function findUserByShareCode(code) {
    try {
        const snapshot = await dbRef.child('users')
...
function initEmptyCharts() {
    ...
    charts.subject = createPieChart('subjectChart', [], [], ['#4c51bf', '#4299e1', '#48bb78', '#ed8936', '#9f7aea']);
}
...
async function handleStudySessionSubmit(e) {
    ...
    try {
        const success = await addStudySession({
            userId: user.uid,
            subject,
            duration,
            date
...
async function handleViewCode() {
    ...
    try {
        ...
        const user = await findUserByShareCode(code);
        if (!user) {
        ...
        displaySharedStats(user.name || 'Anonymous', sessions);
    } catch (error) {
